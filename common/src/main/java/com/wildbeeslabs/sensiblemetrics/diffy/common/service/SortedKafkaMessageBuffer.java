...
import com.wildbeeslabs.sensiblemetrics.diffy.common.interfaces.Buffer;
...
@EqualsAndHashCode
@ToString
public class SortedKafkaMessageBuffer<E extends Comparable> implements Buffer<E> {
    ...
    public SortedKafkaMessageBuffer(int capacity) {
        ValidationUtils.isTrue(capacity > 0, "Capacity may not be <= 0");
    ...
    public void put(E e) throws InterruptedException {
        ValidationUtils.notNull(e, "Element may not be empty");
    ...
    public void putAll(Collection<E> c) throws InterruptedException {
        ValidationUtils.notNull(c, "Element may not be empty");
        final ReentrantLock lock = this.lock;
        lock.lockInterruptibly();
        try {
            for (E e : c) {
