...
public final class TracingInfo {
    ...
    public String[] getMessages() {
        ...
        for (int i = 0; i < messages.length; i++) {
            ...
            // event
            textSB.append(String.format("%-11s ", message.getEvent().getName()));
            // duration
            textSB.append('[')
    ...
    public static class Message {
         ...
         */
        public Message(final Event event, final long duration, final String[] args) {
            this.event = event;
            this.duration = duration;

            this.timestamp = System.nanoTime();
            if (event.getDescription() != null) {
                this.text = String.format(event.getDescription(), (Object[]) args);
            } else {
                final StringBuilder textSB = new StringBuilder();
