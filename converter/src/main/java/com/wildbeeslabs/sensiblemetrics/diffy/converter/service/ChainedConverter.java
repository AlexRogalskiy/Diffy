...
public class ChainedConverter<S, T> implements ContentTypeConverter<S, T> {
     ...
     * @throws ConvertOperationException if no converter can be created using given candidates
     */
    public static <S, T> ChainedConverter<S, T> calculateChain(Class<S> sourceType, Class<T> targetType,
        ...
        if (route == null) {
            throw new ConvertOperationException(format("Cannot build a converter to convert from %s to %s", sourceType.getName(), targetType.getName()));
        }
    ...
    public ChainedConverter(List<ContentTypeConverter<?, ?>> delegates) {
        ValidationUtils.isTrue(delegates != null && !delegates.isEmpty(), "The given delegates may not be null or empty");
        ValidationUtils.isTrue(isContinuous(delegates), "The given delegates must form a continuous chain");
    ...
    private static final class Route {
        ...
        private Route joinedWith(ContentTypeConverter newVertex) {
            ValidationUtils.isTrue(endPoint.equals(newVertex.expectedSourceType()), "Cannot append a vertex if it does not start where the current Route ends");
